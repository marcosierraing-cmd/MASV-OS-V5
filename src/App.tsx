    ...
    return (
      <div style={{ padding: 20, color: S.text }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 20 }}>
          <div style={{ display: "flex", gap: 6, background: S.card, padding: 4, borderRadius: 8, width: "fit-content", border: `1px solid ${S.border}` }}>
            {["Mensual", "Anual"].map(tf => (
              <button key={tf} onClick={() => setTimeFrame(tf)} style={{ padding: "6px 16px", borderRadius: 6, fontSize: 11, fontWeight: 700, border: "none", cursor: "pointer", background: timeFrame === tf ? S.subtle : "transparent", color: timeFrame === tf ? "#fff" : S.muted }}>{tf}</button>
            ))}
          </div>
          <div style={{ display: "flex", gap: 6, background: S.card, padding: 4, borderRadius: 8, width: "fit-content", border: `1px solid ${S.border}` }}>
            {["Simple", "Acumulado"].map(vm => (
              <button key={vm} onClick={() => setViewMode(vm)} style={{ padding: "6px 16px", borderRadius: 6, fontSize: 11, fontWeight: 700, border: "none", cursor: "pointer", background: viewMode === vm ? S.b+"40" : "transparent", color: viewMode === vm ? S.b : S.muted }}>{vm}</button>
            ))}
          </div>
        </div>
        ...
        <Card color={S.g} title={`P&L ${timeFrame} (${viewMode})`}>
          ...
          <div style={{ fontSize: 9, color: S.muted, borderTop: `1px solid ${S.border}`, paddingTop: 8, marginTop: 8 }}>* Mostrando datos en modo <b>{viewMode.toLowerCase()}</b>.</div>
        </Card>

        <Card color={S.y} title="Balance General (Saldos)">
          <div style={{ fontSize: 11, color: S.muted }}>Activos: $240,000 | Pasivos: $80,000 <br/>Capital Contable: $160,000</div>
        </Card>
      </div>
    );
  }
  ...
  if (subTab === "Conciliador SAT") {
    return (
      <div style={{ padding: 20, color: S.text, textAlign: "center" }}>
        ...
        <p style={{ fontSize: 12, color: S.muted, marginTop: 10, lineHeight: 1.6 }}>Aquí arrastraremos el XML/CSV del SAT para cruzarlo con el archivo del Banco. <br/>El sistema buscará facturas huérfanas y pagos sin comprobar.</p>
      </div>
    );
  }
}
  ...
  return (
    <>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 15 }}>
        <div>
          <div style={{ fontSize: 16, fontWeight: 800, display: "flex", alignItems: "center", gap: 10 }}>Auditoría de Día <input type="date" value={selectedDate} onChange={(e) => setSelectedDate(e.target.value)} style={{ background: S.subtle, color: S.text, border: `1px solid ${S.border}`, borderRadius: 6, padding: "4px 8px", fontSize: 11, outline: "none", fontFamily: S.font }} /></div>
...
// MÓDULO 1.2: SEMANAL
// ═══════════════════════════════════════
function TabSemanal({ weeks, updateWeeks, goals }) {
...
// ═══════════════════════════════════════
// MÓDULO 5: PROTOCOLOS (CON LINKS DE YOUTUBE RESTAURADOS)
// ═══════════════════════════════════════
function TabProtocolos() {
  const [protocol, setProtocol] = useState(() => {
    const saved = localStorage.getItem("masv-protocol-v5");
    const defaultP = {
      ayuno: { titulo: "Ayuno Intermitente 16:8", rompe: "Caldo de hueso + Grasas" },
      suplementos: [{ n: "Magnesio", h: "Noche" }, { n: "Creatina", h: "Mañana" }],
      semaforo: { verde: ["Huevo", "Carne", "Vegetales"], rojo: ["Azúcar", "Harinas", "Ultraprocesados"] },
      rutinaAbdomen: ["Plancha", "Deadbug", "Bird-dog"],
      comidas: { semana: ["Proteína + Grasa + Vegetales"], finde: "Flexibilidad controlada" }
    };
    return saved ? JSON.parse(saved) : defaultP;
  });

  const [isEditing, setIsEditing] = useState(false);
  const [activeDay, setActiveDay] = useState(() => { let d = new Date().getDay(); return d === 0 ? 6 : d - 1; });

  const isWeekend = new Date().getDay() === 0 || new Date().getDay() === 6;
  const isFastingDay = new Date().getDay() === 5; 
  const sInput = { width: "100%", background: S.bg, border: `1px solid ${S.border}`, color: "#fff", padding: "8px", borderRadius: "6px", marginBottom: "8px", fontSize: "11px" };
      ...
      {isEditing ? (
        <div style={{ display: "flex", flexDirection: "column", gap: 15 }}>
          ...
          <Card title="🚦 Semáforo Nutricional" color={S.r}>
             <div style={{fontSize:9, color: S.g, marginBottom:4}}>VERDE (Separa por comas)</div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
              {protocol.rutinaAbdomen.map((ex, i) => (
                <div key={i} style={{ background: S.bg, padding: 8, borderRadius: 6, border: `1px solid ${S.border}`, display: "flex", flexDirection: "column", justifyContent: "space-between", minHeight: 45 }}>
                  ...
                  <a href={`https://www.youtube.com/results?search_query=${encodeURIComponent(ex + " tecnica")}`} target="_blank" rel="noreferrer" style={{ fontSize: 8, color: S.muted, textDecoration: "none", background: S.subtle, padding: "2px 5px", borderRadius: 4, display: "inline-flex", alignItems: "center", gap: 4, alignSelf: "flex-start" }}>
...
// ═══════════════════════════════════════
// ESTRATEGIA (RESTAURADO COMPLETO) Y CARD
// ═══════════════════════════════════════
